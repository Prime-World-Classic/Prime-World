...
void LoadingScreenLogic::SetHeroInfo( int userId, const HeroInfo & heroInfo, const NCore::ClientSettings & _clientSettings )
{
  ...
  if ( _clientSettings.showHeroRating )
  {
    ...
    flashInterface->SetHeroRaitingAcc(userId, heroInfo.raitingAcc, heroInfo.winDeltaRaitingAcc, heroInfo.loseDeltaRaitingAcc, heroInfo.isNovice,  rankCalculator->GetRankIcon(faction, rankAcc), rankCalculator->GetRankName(faction, rank));
  }
}
